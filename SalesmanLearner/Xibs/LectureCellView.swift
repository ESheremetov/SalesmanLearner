...
class LectureCellView: UITableViewCell {
    
    
    @IBOutlet weak var lectureIsDoneCheckBox: CheckBox!
    
    @IBOutlet weak var lectureTitleLabel: UILabel!
    @IBOutlet weak var lectureNumberLabel: UILabel!
    
    static let nib = UINib(nibName: String(describing: LectureCellView.self), bundle: nil)
    static let reusableIdentifier = String(describing: LectureCellView.self)
    ...
    private func setupCheckBox() {
        lectureIsDoneCheckBox.style = .square
        lectureIsDoneCheckBox.borderStyle = .roundedSquare(radius: 5.0)
        lectureIsDoneCheckBox.tintColor = UIColor(named: "AccentColor")
    }
}
